...
@PIPELINES.register_module()
class TamperFocusedRandomCrop(RandomCrop):
    """Foreground-aware random crop for tiny tampered regions.

    It tries several random crops and prefers one containing more pixels from
    the target foreground class. This is useful when the positive region is
    very sparse and vanilla random cropping degenerates to background-only
    patches most of the time.
    """
    ...
    def __init__(self,
                 ...
                 num_attempts=20):
        super(TamperFocusedRandomCrop, self).__init__(
            crop_size=crop_size,
    ...
    def _foreground_stats(self, seg_patch):
        valid_mask = seg_patch != self.ignore_index
        valid_pixels = int(valid_mask.sum())
        if valid_pixels == 0:
            return 0, 0.
        foreground_pixels = int((seg_patch == self.foreground_index).sum())
        foreground_ratio = foreground_pixels / float(valid_pixels)
        return foreground_pixels, foreground_ratio

    def __call__(self, results):
        img = results['img']
        gt_seg = results['gt_semantic_seg']

        best_crop_bbox = self.get_crop_bbox(img)
        best_foreground_pixels = -1

        for _ in range(self.num_attempts):
            crop_bbox = self.get_crop_bbox(img)
            seg_patch = self.crop(gt_seg, crop_bbox)
            foreground_pixels, foreground_ratio = self._foreground_stats(
                seg_patch)
            ...
            if foreground_pixels >= self.min_foreground_pixels and \
                    foreground_ratio >= self.min_foreground_ratio:
                best_crop_bbox = crop_bbox
                break

        img = self.crop(img, best_crop_bbox)
        results['img'] = img
        results['img_shape'] = img.shape

        for key in results.get('seg_fields', []):
            results[key] = self.crop(results[key], best_crop_bbox)
    ...
    def __repr__(self):
        return (self.__class__.__name__ +
                ...
                f'foreground_index={self.foreground_index}, '
                f'min_foreground_pixels={self.min_foreground_pixels}, '
                f'min_foreground_ratio={self.min_foreground_ratio}, '
